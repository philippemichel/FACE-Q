---
subtitle: "Plan d'analyse statistique V0.1"
toc: false
lof: false
lot: false
echo: false
...
La cohérence interne sera étudiée à l’aide du coefficient  $\alpha$ de Cronbach (package `psych` [@psych]). Un $\alpha$ supérieur à \num{0.6} sera considéré comme satisfaisant.

## Validité du construit évaluée à l’aide de tests de corrélation

Pour chacune des trois échelles une étude en corrélation multiples entre les items seront réalisées par \gls{pca} (packages `FactoMineR` [@facto] & `factoextra` [@factoextra]) --- figure @fig-pca . Idéalement, vu la construction du score,  tous les items devraient être sur des axes proches & dans des directions proches.
...
Le questionnaire NOSE est un score calculé sur sur 5 items (échelles de Likert) avec un résultat de 0 à 100. Seuls seront pris en compte les scores complets en accord avec les promoteurs du score @lipan13.

Les résultats de ce questionnaire seront présentés. Les corrélations seront ensuite calculées entre le score NOSE  & chacune des échelles du score \gls{faceq}.
